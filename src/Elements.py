...
class AbstractElement:
    def __init__(self, uiManager, container, pos, size, defaultX, defaultY, defaultWidth, defaultHeight, labelPos=None):
        # This way we can accept both Points and tuples and lists.
        pos = [pos[0], pos[1]]
...
class Button(AbstractElement):
    def __init__(self, pos, uiManager, text, func, *params, label=None, size=[None, None], container=None, **kwparams):
        ...
        self.element = pygame_gui.elements.UIButton(relative_rect=pygame.Rect(self.pos, self.size), text=self.text, manager=self.uiManager, container=self.container)
...
class ImageButton(AbstractElement):
    def __init__(self, pos, uiManager, image, func, *params, label=None, size=[None, None], container=None, background=None, deltaColor=30, **kwparams):
        ...
        # self.pos[0] -= (self.image.get_rect().centerx - self.pos[0])
        # self.pos[1] -= (self.image.get_rect().centery - self.pos[1])
        ...
        blend = blend.copy()
        blend.fill([deltaColor, deltaColor, deltaColor, 100])

        disabledImage = image.copy()
        disabledImage.blit(blend, [0, 0], special_flags=pygame.BLEND_RGBA_SUB)
        ...
        backgroundButtonSurf3 = backgroundButtonSurf.copy()
        
        backgroundButtonSurf.blit(image, [0, 0])
        normImage = backgroundButtonSurf

        backgroundButtonSurf2.blit(hoverImage, [0, 0])
        hoverImage = backgroundButtonSurf2
        
        backgroundButtonSurf3.blit(disabledImage, [0, 0])
        disabledImage = backgroundButtonSurf3


        self.element.normal_image = normImage
        self.element.hovered_image = hoverImage
        self.element.selected_image = normImage
        self.element.disabled_image = disabledImage
    ...
    def rebuild(self):
        self.element.kill()
        self.element = pygame_gui.elements.UIButton(relative_rect=pygame.Rect(self.pos, self.image.get_size()), text='', manager=self.uiManager, container=self.container)
        
        # Create a darker image to blend
        blend = pygame.Surface(self.image.get_size(), flags=pygame.SRCALPHA)
        blend.fill([self.deltaColor, self.deltaColor, self.deltaColor, 0])

        hoverImage = self.image.copy()
        hoverImage.blit(blend, [0, 0], special_flags=pygame.BLEND_RGBA_ADD)

        blend = blend.copy()
        blend.fill([self.deltaColor, self.deltaColor, self.deltaColor, 100])

        disabledImage = self.image.copy()
        disabledImage.blit(blend, [0, 0], special_flags=pygame.BLEND_RGBA_SUB)

        # Add the background
        backgroundButtonSurf = pygame.Surface(self.image.get_size())
        if type(self.background) in [list, tuple, pygame.Color]:
            backgroundButtonSurf.fill(self.background)
        elif type(self.background) == pygame.Surface:
            backgroundButtonSurf = self.background
        ...
        backgroundButtonSurf.blit(self.image, [0, 0])
        normImage = backgroundButtonSurf
...
class AnimationButton(AbstractElement):
    def __init__(self, pos, uiManager, animation, func, *params, label=None, size=[None, None], container=None, background=None, deltaColor=30, **kwparams):
        ...
        self.blendSurf.fill([self.deltaColor, self.deltaColor, self.deltaColor, 0])

        self.disabledBlendSurf = pygame.Surface(animation.getSize(), flags=pygame.SRCALPHA)
        self.disabledBlendSurf.fill([self.deltaColor, self.deltaColor, self.deltaColor, 100])
        ...
        self.backgroundButtonSurf3 = self.backgroundButtonSurf.copy()
    ...
    def animate(self):
        self.currentImage = self.animation.animate()

        hoverImage = self.currentImage.copy()
        hoverImage.blit(self.blendSurf, [0, 0], special_flags=pygame.BLEND_RGBA_ADD)

        disabledImage = self.currentImage.copy()
        disabledImage.blit(self.disabledBlendSurf, [0, 0], special_flags=pygame.BLEND_RGBA_SUB)
        
        self.backgroundButtonSurf.blit(self.currentImage, [0, 0])
        self.currentImage = self.backgroundButtonSurf

        self.backgroundButtonSurf2.blit(hoverImage, [0, 0])
        hoverImage = self.backgroundButtonSurf2

        self.backgroundButtonSurf3.blit(disabledImage, [0, 0])
        disabledImage = self.backgroundButtonSurf3
        
        # self.element.normal_image = image
        self.element.hovered_image = hoverImage
        self.element.selected_image = self.currentImage
        self.element.disabled_image = disabledImage

        if self.element.hovered:
            self.element.normal_image = hoverImage
        # elif not self.enabled:
        #     self.element.normal_image = disabledImage
        else:
            self.element.normal_image = self.currentImage
    ...
    def rebuild(self):
        self.element = pygame_gui.elements.UIButton(relative_rect=pygame.Rect(self.pos, self.animation.getSize()), text='', manager=self.uiManager, container=self.container)

        # Create a darker image to blend
        self.blendSurf = pygame.Surface(self.animation.getSize(), flags=pygame.SRCALPHA)
        self.blendSurf.fill([self.deltaColor, self.deltaColor, self.deltaColor, 0])

        self.disabledBlendSurf = self.blendSurf.copy()
        self.disabledBlendSurf.fill([self.deltaColor, self.deltaColor, self.deltaColor, 100])

        # Add the background
        self.backgroundButtonSurf = pygame.Surface(self.animation.getSize())
        if type(self.background) in [list, tuple, pygame.Color]:
            self.backgroundButtonSurf.fill(self.background)
        elif type(self.background) == pygame.Surface:
            self.backgroundButtonSurf = self.background
        elif self.background is None:
            pass
        else:
            assert(False)
        ...
        self.currentImage = self.animation.frames[0]
